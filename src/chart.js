...
const NICE = [0.05, 0.1, 0.2, 0.25, 0.5, 0.75, 1, 1.5, 2, 3, 4, 6, 8, 12, 16, 24, 32]
...
export function plot(series, { width, rows, max = 1, pad = 0 }) {
  ...
  series.forEach((s, si) => {
    ...
    for (let c = 0; c < span; c++) {
      ...
      // A zero is a HOLE, not a point on the floor.
      //
      // Drawn as a baseline value, every idle project lays a solid line along
      // the bottom row for every hour it was not touched, and six of those on
      // top of each other is the whole width of the chart in noise. The
      // reference charts this imitates plot scores, which always have a value;
      // attention does not, and "nothing happened here" is exactly what a gap
      // in a line already means to anyone reading a chart.
      if (!(v > 0)) { prev = null; continue }
      const y = rowOf(v)
      // The first column of every run gets the MARKER, not whatever the
      // pattern happens to land on. Editing is bursty: a four-minute burst is
      // one column wide, and a series whose only column fell on a `-` drew a
      // dash indistinguishable from three other series. The glyph that
      // identifies the line has to be the one that always appears.
